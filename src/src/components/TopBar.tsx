...
export function TopBar({
  ...
  const handleViewSelect = (viewId: string) => {
    ...
    if (viewToTabMap[viewId]) {
      // Use existing tab
      onTabChange(viewToTabMap[viewId]);
            ...
            {activeTab === tab.id ? (
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <button
                    className={`px-4 py-2 rounded-md transition-colors flex items-center gap-2 text-sm bg-accent text-accent-foreground hover:bg-accent`}
                  >
                    {tab.label}
