...
// =============================================
// components/Navigation/TopBar.tsx (refactored)
// =============================================
...
import { Table, BarChart3, Calendar, FileText, FolderOpen, FormInput, Layers, MoreHorizontal, Terminal, Code, TestTube } from "lucide-react";
...
export function TopBar({
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      portals.forEach((portal) => {
        ...
        if (themeClasses) themeClasses.split(" ").forEach((cls) => cls && portal.classList.add(cls));
      });
    }, 0);
  }, [getThemeClasses]);
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      try { setApiConfig(JSON.parse(savedConfig)); } catch { /* ignore */ }
    }
  }, []);
  ...
  const handleApiConfigChange = (newConfig: ApiConfig) => {
    ...
    toast.success('API configuration saved successfully!', { description: 'Your Frappe ERPNext connection settings have been updated.' });
  };
  ...
  const handleTestConnection = async (config: ApiConfig): Promise<boolean> => {
    try {
      ...
      const response = await fetch(url, { method: 'GET', headers: { 'Authorization': `token ${config.token}`, 'Content-Type': 'application/json' }, credentials: 'omit', signal: AbortSignal.timeout(config.timeout) });
      if (response.ok) { const data = await response.json(); return data && (Array.isArray(data.data) || data.message); }
      return false;
    } catch { return false; }
  };
  ...
  const developerTools: ViewItem[] = [
    { id: "developer", label: "Developer Dashboard", icon: Terminal, badge: "Dev", description: "CSRF tokens, session info, and debug tools" },
    { id: "hacker-pro", label: "Hacker Pro", icon: Code, badge: "Pro", description: "Bulk ticket generation with demo data" },
    { id: "tester", label: "API Tester", icon: TestTube, badge: "Test", description: "Comprehensive API testing suite with live results" },
  ];

  const handleViewSelect = (viewId: string) => {
    const viewToTabMap: Record<string, string> = { table: "main-table", kanban: "kanban", form: "form" };
    const mappedTabLabels: Record<string, string> = { "main-table": "Main table", kanban: "Kanban", form: "Form" };
    const viewLabels: Record<string, string> = { chart: "Charts", calendar: "Calendar", gantt: "Gantt", doc: "Documents", "file-gallery": "File Gallery", customizable: "Custom View", developer: "Developer Dashboard", "hacker-pro": "Hacker Pro", tester: "API Tester" };

    if (viewToTabMap[viewId]) {
      const mappedTabId = viewToTabMap[viewId];
      const existing = tabs.find((t) => t.id === mappedTabId);
      if (!existing) onAddTab(mappedTabId, mappedTabLabels[mappedTabId] || viewId);
  ...
  const getAccentColorClass = () => "bg-theme-accent hover:bg-theme-accent-hover text-theme-accent-foreground mytick-theme";
  const getBadgeAccentClass = () => "bg-theme-accent text-theme-accent-foreground border-none mytick-theme";
  ...
  const handleTooltipToggle = (checked: boolean) => {
    ...
    toast.success(checked ? "Tooltips enabled" : "Tooltips disabled", { description: checked ? "Helpful hints will now show on hover" : "Tooltips have been disabled for a cleaner interface" });
  };
      ...
      <header className="bg-card border-b border-border px-6 py-3 flex items-center justify-between mytick-theme">
        ...
        <div className="flex items-center gap-4 flex-shrink-0 mytick-theme">
          <div className="flex items-center gap-4 mytick-theme">
            <h1 className="text-xl font-medium text-foreground mytick-theme">MYTICK</h1>
            <Button variant="outline" size="sm" onClick={toggleEditable}>{isEditable ? "Lock layout" : "Unlock layout"}</Button>
            <Badge variant="secondary" className={getBadgeAccentClass()}>In Portal</Badge>
          </div>
